...
public class DefaultPriceService implements PriceService {
	...
	Flux<Map<String, Object>> selectOnlyPriceUpdateEvents(Flux<Map<String, Object>> input) {
		...
		return input
				.filter(stringObjectMap -> stringObjectMap.containsKey(MessageMapper.PRICE_KEY))
	...
	Flux<MessageDTO<Float>> averagePrice(Flux<Long> requestedInterval,
		...
		return requestedInterval
				...
				.switchMap(aLong -> priceData.window(Duration.ofSeconds(aLong)))
				.flatMap(messageDTOFlux ->
						messageDTOFlux
								.groupBy(MessageDTO::getCurrency)
								.flatMap(groupedFlux ->
									groupedFlux.map(MessageDTO::getData)
											.reduce(Sum.empty(), Sum::add)
											.map(Sum::avg)
											.map(avg -> MessageDTO.avg(avg, groupedFlux.key(), "Avg"))
										)
				);
	}
}
